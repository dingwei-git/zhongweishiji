...
import com.jovision.jaws.common.properties.RedisProperties;
...
public class RedisUtil extends RedisProperties {
    ...
    public void initialPool() {
        try {
            ...
            config.setMaxTotal(Integer.valueOf(getRedismaxactive()));
            config.setMaxIdle(Integer.valueOf(getRedismaxidle()));
            config.setMaxWaitMillis(Integer.valueOf(getRedismaxwait()));
            ...
            jedisPool = new JedisPool(config, getRedisip().split(",")[0], Integer.valueOf(getRedisport()), Integer.valueOf(getRedistimeout()), getRedispass());
        } catch (Exception e) {
            /*try{
                //如果第一个IP异常，则访问第二个IP
                JedisPoolConfig config = new JedisPoolConfig();
