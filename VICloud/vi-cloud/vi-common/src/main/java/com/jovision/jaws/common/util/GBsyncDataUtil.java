...
public class GBsyncDataUtil {
    ...
    public static List<JSONObject> syncData(GBsyncData gBsyncData) {
        ...
        getDataFromIVS3800(result,gBsyncData);
    ...
    private static void getDataFromIVS3800(List<JSONObject> result, GBsyncData gBsyncData) {
        Map<String, Object> bodyMap = new HashMap<>();
        bodyMap.put("platformid", gBsyncData.getPlatformId());
        String body = JSON.toJSONString(bodyMap);
        int len = body.getBytes().length;
        ...
        try {
            ...
            result.addAll(objects);
            if(result.size()>1){
                JSONObject baseObject = result.get(0);
                JSONObject deviceListObject = baseObject.getJSONObject("DeviceList");
                //DeviceList中设备的数量
                ...
                for(int i=1;i<result.size();i++){
                    JSONObject after = result.get(i).getJSONObject("DeviceList");
                    //DeviceList中设备的数量
                    JSONArray afternew = new JSONArray();
                    try {
                        afternew.addAll(after.getJSONArray("Item"));
                    } catch (Exception e) {
                        afternew.add(after.getJSONObject("Item"));
                    }
                    devices.addAll(afternew);
                }

                baseObject.getJSONObject("DeviceList").put("Item",devices);
                result.clear();
