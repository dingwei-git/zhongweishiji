...
public class CollectionTimer implements Runnable{
     ...
     */
    public CollectionTimer(Date periodStarTime, List<TblServerParamConfig> serverParam,
    ...
    public void run() {
        ...
        if (nowTime - starTime.getTime() >= periodTime) {
            ...
            if (ivsServ.queryIvsServerParam() > 0) {
                ivsServ.ivsCollect(serverParamInfos, starTime);
     ...
     * 采集定时器设置
     *
     * @param collectionTimer 分析定时器
     */
    public static synchronized void setCollectionTimer(CollectionTimer collectionTimer) {
        CollectionTimer.collectionTimer = collectionTimer;
    }
